...
const ParticularPgPage = ({ dashboardData, selectPg, setOpen, open }) => {
  ...
  React.useEffect(() => {
    ...
    if (dashboardData?.[0]?.properties) {
      const selected = dashboardData[0].properties.find((obj) => obj.id == selectPg);
      if (selected) {
        setPg(selected);
  ...
  return (
    <Dialog
      ...
      fullWidth
    >
      <DialogTitle id="pg-dialog-title" sx={{fontSize:"2rem",fontWeight:700,display:"flex",justifyContent:"space-between"}}>
        <div>
            {pg?.propertyName || "PG Details"} <span><Chip label={pg?.isCoed?"Co-ed PG":"No Co-ed PG"} /></span>
        </div>
        <div>
        ...
        <DialogContentText id="pg-dialog-description">
          <div className='conatiner-fluid'>
            <div className="row">
                <div className="col-6">
                    <table className="table">
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td>isFurninshed</td>
                                <td>{pg?.isFurninshed?"Full furninshed":"Unfurnished"}</td>
                            </tr>
                ...
                <div className="col-6">
                    <Card sx={{display:"flex",flexDirection:"column",alignItems:"center",gap:2}} elevation={3}>
                        <h6>Expenses</h6>
                        <div className='w-full'>
                            <div className='flex justify-around'>
                                <div className='flex justify-start  w-[50%] text-center'>Rent</div>
                                <div>{pg?.rent}</div>
                            </div>

                            <div className='flex justify-around'>
                                <div className='flex justify-start  w-[50%] text-center'>Security-Money</div>
                                <div>{pg?.securityMoney}</div>
                            </div>
                            <hr />
                            <div className='flex justify-around'>
                                <div className='flex justify-start  w-[50%] text-center'>Total</div>
